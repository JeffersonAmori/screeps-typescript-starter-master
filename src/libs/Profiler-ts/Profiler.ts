...
function record(key: string, time: number) {
  if (!Memory.profiler.map[key]) {
    Memory.profiler.map[key] = {
      calls: 0,
      time: 0,
    };
  }
  Memory.profiler.map[key].calls++;
  Memory.profiler.map[key].time += time;
}
...
function outputProfilerData() {
  ...
  const data = Reflect.ownKeys(Memory.profiler.map).map((key) => {
    calls = Memory.profiler.map[key].calls;
    time = Memory.profiler.map[key].time;
    result = {};
