...
export class MotherProcess extends Process {
  ...
  createCreeps(): void {
    ...
    if (Game.flags.claimFlag) {
      const diplomats = _.filter(Game.creeps, (c) => c.memory.role === Consts.roleDiplomat);
      if (diplomats.length < Consts.maxNumberDiplomats) {
        creepFactory.CreateCreep(Consts.roleDiplomat);
      }
    }
  }
